...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        x_out=(x-running_mean)/(np.sqrt(running_var+eps))
...
def batchnorm_backward_alt(dout, cache):
    ...
    dsample_mean=np.sum(dx_hat*(-1)/np.sqrt(sample_var+eps),axis=0)+dsample_var*(-2/N)*np.sum(x-sample_mean,axis=0)
    dx=dx_hat/np.sqrt(sample_var+eps)+dsample_var*(2/N)*(x-sample_mean)+dsample_mean/N
    dgamma=np.sum(dout*x_hat,axis=0)
    dbeta=np.sum(dout,axis=0)
    pass
    ###########################################################################
    ...
    #for i in range(F):
        #for j in range(H_prime):
            #for k in range(W_prime):
                #out[:,i,j,k]=np.sum(x_pad[:,:,j*stride:(j*stride+HH),k*stride:(k*stride+WW)]*w[i,:,:,:],axis=(1,2,3))
    for i in range(H_prime):
      for j in range(W_prime):
          x_padded_mask = x_padded[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
          for k in range(F):
              for n in range(N):
                  out[n, k, i, j] = np.sum(x_padded_mask[n,:,:,:] * w[k, :, :, :])
              #print(out[:, k, i, j].shape)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_pool=x[:,:, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            dx_pool=dx[:,:, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            #mask = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            max_position_flag=np.max(x_pool, axis=(2, 3),keepdims=True)==x_pool
            dx_pool+=max_position_flag * (dout[:, :, i, j])[:, :, None, None]
            dx[:,:, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]=dx_pool
