...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss=-np.sum(np.log(prob_new[np.arange(N),list(y)]))
    loss/=N
    ...
    #############################################################################
    prob_for_dscore=prob_new
    prob_for_dscore[np.arange(N),list(y)]-=1
    dscore=prob_for_dscore/N
    grads['W2']=np.dot(a1.T,dscore)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      batch_indices=np.random.choice(num_train,200)
  ...
  def predict(self, X):
    ...
    prob_sum=np.sum(prob,axis=1).reshape(-1,1)
    prob_new=prob/prob_sum
    y_pred=np.argmax(prob_new,axis=1)
    pass
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
